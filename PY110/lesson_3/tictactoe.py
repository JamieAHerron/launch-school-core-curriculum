import random
import os

INITIAL_MARKER = ' '
HUMAN_MARKER = 'X'
COMPUTER_MARKER = 'O'
WINNING_SCORE = 5
WINNING_LINES = [
        [1, 2, 3], [4, 5, 6], [7, 8, 9], #rows
        [1, 4, 7], [2, 5, 8], [3, 6, 9], #columns
        [1, 5, 9], [3, 5, 7]             #diagonals 
    ]

def prompt(message):
    print(f'===> {message}')

def display_board(board, player, computer):
    os.system('clear')

    prompt(f"You are {HUMAN_MARKER}. Computer is {COMPUTER_MARKER}.")
    print('')
    print('     |     |')
    print(f"  {board[1]}  |  {board[2]}  |  {board[3]}")
    print('     |     |')
    print('-----+-----+-----')
    print('     |     |')
    print(f"  {board[4]}  |  {board[5]}  |  {board[6]}")
    print('     |     |')
    print('-----+-----+-----')
    print('     |     |')
    print(f"  {board[7]}  |  {board[8]}  |  {board[9]}")
    print('     |     |')
    print('')
    print(f"Player: {player} Computer: {computer}")

def join_or(choices, delimiter=',', final_word='or'):
    first_items = choices[:-1]
    last_item = choices[len(choices) - 1]

    if len(choices) > 1:
        return f"{delimiter.join(first_items)} {final_word} {last_item}"
    elif len(choices) == 1:
        return f"{delimiter.join(choices)}"
    elif choices == []:
        return ""

def initialize_board():
    return {square: INITIAL_MARKER for square in range(1, 10)}

def empty_squares(board):
    return [key for key, value in board.items() if value == INITIAL_MARKER]

def player_chooses_square(board):
    while True:
        valid_choices = [str(num) for num in empty_squares(board)]
        prompt(f"Choose a square ({join_or(valid_choices)}):")
        square = input().strip()
        if square in valid_choices:
            break
        
        prompt("Sorry, that's not a valid choice.")
    
    board[int(square)] = HUMAN_MARKER

def computer_chooses_square(board):
    if len(empty_squares(board)) == 0:
        return
    square = random.choice(empty_squares(board))
    board[square] = COMPUTER_MARKER

def board_full(board):
    return len(empty_squares(board)) == 0

def someone_won(board):
    return bool(detect_winner(board))

def detect_winner(board):
    for line in WINNING_LINES:
        sq1, sq2, sq3 = line
        if (board[sq1] == HUMAN_MARKER
             and board[sq2] == HUMAN_MARKER
             and board[sq3] == HUMAN_MARKER):
            
            return 'Player'
        elif (board[sq1] == COMPUTER_MARKER
                  and board[sq2] == COMPUTER_MARKER
                  and board[sq3] == COMPUTER_MARKER):
            return 'Computer'
    
    return None

def play_tic_tac_toe():
    player_score = 0
    computer_score = 0

    while True:
        board = initialize_board()

        while True:
            display_board(board, player_score, computer_score)

            player_chooses_square(board)
            if someone_won(board) or board_full(board):
                break

            computer_chooses_square(board)
            if someone_won(board) or board_full(board):
                break
        
        display_board(board, player_score, computer_score)

        winner = detect_winner(board)
        if winner == 'Player':
            player_score += 1
            prompt("You won this round!")
        elif winner == 'Computer':
            computer_score += 1
            prompt("The computer won this round!")
        else:
            prompt("It's a tie!")
        
        prompt(f"Score - Player: {player_score} Computer: {computer_score}")

        if player_score == WINNING_SCORE:
            prompt("Congratulations, you won the match!")
            break
        elif computer_score == WINNING_SCORE:
            prompt("The computer won the match. Better luck next time!")
            break

        prompt("Play again? (y or n)")
        answer = input().lower()

        while len(answer) == 0:
            print("Your input is invalid.")
            prompt("Play again? (y or n)")
            answer = input().lower()

        if answer[0] != 'y':
            break
    
    prompt("Thank's for playing Tic Tac Toe!")

play_tic_tac_toe()

